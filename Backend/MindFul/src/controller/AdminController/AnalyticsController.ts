...
export const getAnalytics = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const onlineUsers = users.filter(
      (u) => resolveStatus(u.status, u.lastSeen) === 'online'
    ).length;
    const onlineAdmins = admins.filter(
      (a) => resolveStatus(a.status, a.lastSeen) === 'online'
    ).length;
    ...
    // ---------- Mood distribution ----------
    const moodCounts: Record<string, { count: number; emoji: string; color: string }> = {};
    journals.forEach((j) => {
    ...
    const journalTrendMap: Record<string, number> = {};
    ...
    journals.forEach((j) => {
      ...
      if (journalTrendMap[key] !== undefined) journalTrendMap[key] += 1;
    });
    users.forEach((u) => {
      const key = new Date(u.createdAt).toISOString().slice(0, 10);
      if (userTrendMap[key] !== undefined) userTrendMap[key] += 1;
    });
